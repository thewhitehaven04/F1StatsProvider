...
def store_race_telemetry(season: int):
                ...
                for driver in drivers:
                    ...
                    for lap_number in lap_numbers:
                        ...
                        try:
                            ...
                            telemetry.brake = telemetry.brake.transform(
                                lambda x: int(x)
                            )
                            telemetries.extend(telemetry.to_dict(orient="records"))
                        except:
    ...
    with open(f"telemetry_race_{season}.json", "w") as file:
        file.write(json.dumps(telemetries))


def insert_race_telemetry(season: int):
    postgres = create_engine(
        "postgresql://germanbulavkin:postgres@127.0.0.1:5432/postgres"
    )
    with open(f"telemetry_race_{season}.json", "r") as file:
        file_read = json.loads(file.read())
    ...
    with postgres.connect() as pg_con:
        for batch in batched(file_read, 10000): 
